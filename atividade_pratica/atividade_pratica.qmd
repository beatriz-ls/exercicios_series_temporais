...
```{ma function, warning = FALSE}

ma <- function(x, order) {
  if (!is.numeric(x)) stop("x must be numeric")
  if (!is.numeric(order) || order <= 0) stop("order must be a positive integer")
  
  filt <- rep(1/order, order)  # Criando o filtro de médias móveis
  ma_values <- stats::filter(x, filt, sides = 2)  # Aplicando a média móvel com a função nativa
...
ma_6 <- ma(data_fit$Ubatuba, order = 6)
ma_12 <- ma(data_fit$Ubatuba, order = 12)
...
ma_6 <- ma(data_ts, order = 6)
ma_12 <- ma(data_ts, order = 12)
